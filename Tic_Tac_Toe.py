...
def first():
    ...
    r1 = Radiobutton(
        top, text="X", variable=v1, value="X", command=lambda: XorO()
    ).grid(column=2, row=1)

    r2 = Radiobutton(
        top, text="O", variable=v1, value="O", command=lambda: XorO()
    ).grid(column=2, row=2)

    Widget2 = Label(top, text="Select Type of game").grid(column=2, row=4)

    r3 = Radiobutton(
        top, text="AI", variable=v2, value=1, command=lambda: AIor2P()
    ).grid(column=2, row=5)

    r4 = Radiobutton(
        top, text="Two Players", variable=v2, value=2, command=lambda: AIor2P()
    ).grid(column=2, row=6)

    button = Button(top, text="Ok", command=lambda: Bord(top)).grid(column=2, row=7)
    button2 = Button(top, text="Cancel", command=lambda: quit()).grid(column=2, row=8)
...
def endgame(player, window):
    ...
    button = Button(
        winWindow, text="Play Again", command=lambda: reset(window, winWindow)
    )
...
def checkwin(listToCheck):
    return (
        (
            listToCheck[0]["text"] == listToCheck[1]["text"]
            and listToCheck[0]["text"] == listToCheck[2]["text"]
        )
        or (
            listToCheck[3]["text"] == listToCheck[4]["text"]
            and listToCheck[3]["text"] == listToCheck[5]["text"]
        )
        or (
            listToCheck[6]["text"] == listToCheck[7]["text"]
            and listToCheck[6]["text"] == listToCheck[8]["text"]
        )
        or (
            listToCheck[0]["text"] == listToCheck[3]["text"]
            and listToCheck[0]["text"] == listToCheck[6]["text"]
        )
        or (
            listToCheck[1]["text"] == listToCheck[4]["text"]
            and listToCheck[1]["text"] == listToCheck[7]["text"]
        )
        or (
            listToCheck[2]["text"] == listToCheck[5]["text"]
            and listToCheck[2]["text"] == listToCheck[8]["text"]
        )
        or (
            listToCheck[0]["text"] == listToCheck[4]["text"]
            and listToCheck[0]["text"] == listToCheck[8]["text"]
        )
        or (
            listToCheck[2]["text"] == listToCheck[4]["text"]
            and listToCheck[2]["text"] == listToCheck[6]["text"]
        )
    )
...
def play(position, window):
    ...
    if gameType == 1:
        if (
            listOfButtons[position]["text"] != "X"
            and listOfButtons[position]["text"] != "O"
        ):
